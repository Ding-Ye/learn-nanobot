...
// Runner — the inner LLM-↔-tools loop carved out in s04.
// Mirrors upstream `nanobot/agent/runner.py::AgentRunner`. UNCHANGED in
// s06 — the Bus just calls runner.Run from inside each session goroutine,
// then the bus persists the new turn via MemoryStore.
