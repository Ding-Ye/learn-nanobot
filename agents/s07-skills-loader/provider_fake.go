...
// No network, no API key. Reused unchanged here in s07 — the new SkillLoader
// tests don't actually call Chat (they exercise file walking + YAML parsing
// directly), but we keep FakeProvider so main.go still compiles and so
// future sessions can keep extending the same scaffolding.
