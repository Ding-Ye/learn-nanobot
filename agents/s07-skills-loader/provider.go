...
// Provider — same shape as s03/s04/s05/s06. Carried over verbatim because each
// session is its own self-contained Go module with no cross-imports.
//
...
// Message stays the same as s01-s06 — a turn in the conversation.
type Message struct {
	Role    string         `json:"role"`
	Content []ContentBlock `json:"content"`
}

// ContentBlock — same shape as s01-s06.
...
// Usage is the normalized token accounting. Carried by both LLMResponse and
// HistoryEntry — s06's MemoryStore persists it as part of each turn's record.
type Usage struct {
